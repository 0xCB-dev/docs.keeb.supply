...
The reset buttons, trrs jacks and controllers all go on the top side of the keyboard. That is the side, where the HotSwap sockets are NOT on.
{{< /callout >}}
...
{{< callout context="note" title="Note" icon="outline/info-circle" >}}
The top two through holes, labelled + and -, are for batteries. If you are using your DASBOB with a wired connection, please do not solder these to the controller.
{{< /callout >}}
...
{{< callout context="note" title="Note" icon="outline/info-circle" >}}
Do not hotplug the TRRS Cable, when your controllers are plugged into your PC. This can and will damage the board. Always unplug the keyboard from the PC, before plugging the TRRS Cable in.
{{< /callout >}}
![matrix](matrix.png)
...
{{< callout context="note" title="Note" icon="outline/info-circle" >}}
If you decide to not solder in the hot swap sockets, you will need to solder in the switches later!
{{< /callout >}}
![kailh](dasbob-hs.png)
...
{{< callout context="note" title="Note" icon="outline/info-circle" >}}
With the default firmware that we provide, you need to solder the buzzer onto the left side!
{{< /callout >}}
![buzzer](buzzer.png)
